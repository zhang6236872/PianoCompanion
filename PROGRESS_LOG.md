...
- 视奏练习生成器 UI 集成 (SightReadingGenerator 引擎已就绪, 需在 LibraryScreen 添加生成入口: 调号/难度/小节数选择器 → 生成 Score → 复用现有渲染/跟音/分析管线)
