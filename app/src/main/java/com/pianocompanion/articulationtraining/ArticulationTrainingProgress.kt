...
/**
 * 演奏法辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class ArticulationTrainingProgress(
    val stats: MutableMap<String, ArticulationTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { ArticulationTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: ArticulationTrainingDifficulty): ArticulationTrainingProgressEntry =
        stats[difficulty.name] ?: ArticulationTrainingProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): ArticulationTrainingProgress {
            val result = ArticulationTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = ArticulationTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class ArticulationTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): ArticulationTrainingProgressEntry? {
            return try {
