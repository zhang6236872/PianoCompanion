...
/**
 * 音程听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 记录跨会话的练习进度：每种难度 + 播放方向的累计统计。
 * 通过 [toJson] / [fromJson] 进行 JSON 序列化（手动实现，无外部依赖，容错解析）。
 *
 * @param stats 各难度+方向的统计映射，键格式为 "BEGINNER_ASCENDING"
 */
data class IntervalTrainingProgress(
    val stats: MutableMap<String, IntervalTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val key = key(difficulty, playDirection)
        val entry = stats.getOrPut(key) { IntervalTrainingProgressEntry() }
    ...
    /** 获取指定组合的进度统计。 */
    fun getProgress(difficulty: IntervalDifficulty, playDirection: PlayDirection): IntervalTrainingProgressEntry =
        stats[key(difficulty, playDirection)] ?: IntervalTrainingProgressEntry()
    ...
    companion object {
        fun key(difficulty: IntervalDifficulty, playDirection: PlayDirection): String =
            "${difficulty.name}_${playDirection.name}"

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): IntervalTrainingProgress {
            val result = IntervalTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = IntervalTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度+方向组合的进度统计。
 */
data class IntervalTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): IntervalTrainingProgressEntry? {
            return try {
