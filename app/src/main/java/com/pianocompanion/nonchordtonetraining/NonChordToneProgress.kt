...
/**
 * 和弦外音辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class NonChordToneProgress(
    val stats: MutableMap<String, NonChordToneProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
     ...
     */
    fun recordSession(
        difficulty: NonChordToneDifficulty,
        correct: Int,
        total: Int,
        bestStreak: Int
    ) {
        val key = difficulty.name
        val entry = stats.getOrPut(key) { NonChordToneProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: NonChordToneDifficulty): NonChordToneProgressEntry =
        stats[difficulty.name] ?: NonChordToneProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): NonChordToneProgress {
            val result = NonChordToneProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = NonChordToneProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class NonChordToneProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): NonChordToneProgressEntry? {
            return try {
