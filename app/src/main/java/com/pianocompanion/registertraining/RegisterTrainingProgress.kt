...
/**
 * 音区辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class RegisterTrainingProgress(
    val stats: MutableMap<String, RegisterTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { RegisterTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: RegisterTrainingDifficulty): RegisterTrainingProgressEntry =
        stats[difficulty.name] ?: RegisterTrainingProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): RegisterTrainingProgress {
            val result = RegisterTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = RegisterTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class RegisterTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): RegisterTrainingProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = RegisterTrainingProgressEntry()
