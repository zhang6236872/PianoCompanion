...
/**
 * 调内音级辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class ScaleDegreeProgress(
    val stats: MutableMap<String, ScaleDegreeProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { ScaleDegreeProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: ScaleDegreeDifficulty): ScaleDegreeProgressEntry =
        stats[difficulty.name] ?: ScaleDegreeProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): ScaleDegreeProgress {
            val result = ScaleDegreeProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = ScaleDegreeProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class ScaleDegreeProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): ScaleDegreeProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = ScaleDegreeProgressEntry()
