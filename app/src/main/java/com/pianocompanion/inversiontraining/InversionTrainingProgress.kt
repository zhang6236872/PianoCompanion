 ...
 */
data class InversionTrainingProgress(
    val stats: MutableMap<String, InversionTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
     ...
     */
    fun recordSession(
        difficulty: InversionDifficulty,
        correct: Int,
        total: Int,
        bestStreak: Int
    ) {
        val key = key(difficulty)
        val entry = stats.getOrPut(key) { InversionTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: InversionDifficulty): InversionTrainingProgressEntry =
        stats[key(difficulty)] ?: InversionTrainingProgressEntry()
    ...
    companion object {
        fun key(difficulty: InversionDifficulty): String = difficulty.name

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): InversionTrainingProgress {
            val result = InversionTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = InversionTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class InversionTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): InversionTrainingProgressEntry? {
            return try {
