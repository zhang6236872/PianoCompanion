 ...
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [InversionTrainingAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止、重播。资源管理：使用完毕必须调用 [release]。
 *
 * @param sampleRate 采样率
 */
class InversionTrainingPlayer(
    private val sampleRate: Int = InversionTrainingAudioBuilder.SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "InversionTrainingPlayer"
        private const val POLL_INTERVAL_MS = 50L
        private const val sampleRateStatic = InversionTrainingAudioBuilder.SAMPLE_RATE
