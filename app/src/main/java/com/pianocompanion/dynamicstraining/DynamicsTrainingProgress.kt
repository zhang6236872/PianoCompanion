...
/**
 * 力度辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class DynamicsTrainingProgress(
    val stats: MutableMap<String, DynamicsTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { DynamicsTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: DynamicsTrainingDifficulty): DynamicsTrainingProgressEntry =
        stats[difficulty.name] ?: DynamicsTrainingProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): DynamicsTrainingProgress {
            val result = DynamicsTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = DynamicsTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class DynamicsTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): DynamicsTrainingProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = DynamicsTrainingProgressEntry()
