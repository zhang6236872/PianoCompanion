...
/**
 * 和弦转位听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class ChordInversionProgress(
    val stats: MutableMap<String, ChordInversionProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { ChordInversionProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: ChordInversionDifficulty): ChordInversionProgressEntry =
        stats[difficulty.name] ?: ChordInversionProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): ChordInversionProgress {
            val result = ChordInversionProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = ChordInversionProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class ChordInversionProgressEntry(
    ...
    companion object {
         ...
         */
        fun fromJson(json: String): ChordInversionProgressEntry? {
            return try {
