...
/**
 * 复调运动辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class MotionProgress(
    val stats: MutableMap<String, MotionProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { MotionProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: MotionDifficulty): MotionProgressEntry =
        stats[difficulty.name] ?: MotionProgressEntry()
 ...
 * 单个难度的进度统计。
 */
data class MotionProgressEntry(
    ...
    companion object {
         ...
         */
        fun fromJson(json: String): MotionProgressEntry? {
            return try {
