...
/**
 * 和弦听辨训练音频播放器（Android 层）。
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [ChordTrainingAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止、重播。资源管理：使用完毕必须调用 [release]。
 *
 * @param sampleRate 采样率
 */
class ChordTrainingPlayer(
    private val sampleRate: Int = ChordTrainingAudioBuilder.SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "ChordTrainingPlayer"
        private const val POLL_INTERVAL_MS = 50L
        private val COMPLETION_TOLERANCE_SAMPLES = sampleRateStatic / 50
        private const val sampleRateStatic = ChordTrainingAudioBuilder.SAMPLE_RATE
    }
}
