...
/**
 * 挂留和弦品质听辨训练出题引擎（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 根据 [SuspendedChordDifficulty] 生成 [SuspendedChordQuestion]：
 * 1. 从该难度的可用品质集合中随机选择一个品质
 ...
 * @param root 底层随机数生成器
 */
class SuspendedChordTrainingEngine(
     ...
     * @return 生成的题目
     */
    fun generate(difficulty: SuspendedChordDifficulty): SuspendedChordQuestion {
        val availableQualities = SuspendedChordQuality.forDifficulty(difficulty)

        val quality = availableQualities.random(root)

        // 随机选择根音，限定在 C3-G3 范围（保证和弦在钢琴音域内）
        val rootPc = ROOT_PITCH_CLASSES.random(root)
        val rootMidi = BASE_OCTAVE_MIDI + rootPc
        val rootName = ROOT_NAMES[rootPc] ?: "C"

        // 构建和弦 MIDI 音符列表（从低到高排列）
        val midiNotes = buildSuspendedChordMidiNotes(quality, rootMidi)
    ...
    companion object {
        ...
        /** 可用根音音级类（C, D, E, F, G），限定最高到 G 保证所有音在音域内。 */
        ...
        fun withSeed(seed: Long): SuspendedChordTrainingEngine =
            SuspendedChordTrainingEngine(Random(seed))
         ...
         */
        fun buildSuspendedChordMidiNotes(
