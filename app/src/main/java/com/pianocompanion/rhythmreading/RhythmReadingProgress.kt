...
/**
 * 节奏视读训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 记录跨会话的练习进度：每种难度的累计统计。
 * 通过 [toJson] / [fromJson] 进行 JSON 序列化（手动实现，无外部依赖）。
 *
 * @param stats 各难度的统计映射，键格式为 "BEGINNER" / "INTERMEDIATE" / "ADVANCED"
 */
data class RhythmReadingProgress(
    val stats: MutableMap<String, RhythmReadingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { RhythmReadingProgressEntry() }
     ...
     * 获取指定难度的进度统计。
     */
    fun getProgress(difficulty: RhythmReadingDifficulty): RhythmReadingProgressEntry =
        stats[difficulty.name] ?: RhythmReadingProgressEntry()
    ...
    companion object {
         ...
         * 从 JSON 字符串反序列化。
         */
        fun fromJson(json: String): RhythmReadingProgress {
            val result = RhythmReadingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = RhythmReadingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度组合的进度统计。
 */
data class RhythmReadingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): RhythmReadingProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = RhythmReadingProgressEntry()
