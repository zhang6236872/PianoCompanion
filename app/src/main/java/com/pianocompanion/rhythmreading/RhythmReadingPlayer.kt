...
/**
 * 节奏视读训练音频播放器（Android 层）。
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [RhythmReadingAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止、重播。
 *
 ...
 */
class RhythmReadingPlayer(
    private val sampleRate: Int = RhythmReadingAudioBuilder.SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "RhythmReadingPlayer"
        private const val POLL_INTERVAL_MS = 50L
        private const val sampleRateStatic = RhythmReadingAudioBuilder.SAMPLE_RATE
