...
/**
 * 摇摆感辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class SwingProgress(
    val stats: MutableMap<String, SwingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { SwingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: SwingDifficulty): SwingProgressEntry =
        stats[difficulty.name] ?: SwingProgressEntry()
...
data class SwingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): SwingProgressEntry? {
            return try {
