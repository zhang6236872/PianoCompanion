...
/**
 * 声部进入顺序辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class VoiceEntryProgress(
    val stats: MutableMap<String, VoiceEntryProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { VoiceEntryProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: EntryDifficulty): VoiceEntryProgressEntry =
        stats[difficulty.name] ?: VoiceEntryProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): VoiceEntryProgress {
            val result = VoiceEntryProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = VoiceEntryProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class VoiceEntryProgressEntry(
    ...
    companion object {
         ...
         */
        fun fromJson(json: String): VoiceEntryProgressEntry? {
            return try {
                ...
                val entry = VoiceEntryProgressEntry()
