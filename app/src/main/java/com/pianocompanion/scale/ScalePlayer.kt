 ...
 * 音阶音频播放器（Android 层）。
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [ScaleAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止、重播。资源使用完毕必须调用 [release]。
 */
class ScalePlayer(
    private val sampleRate: Int = ScaleAudioBuilder.SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "ScalePlayer"
        private const val POLL_INTERVAL_MS = 50L
        private const val sampleRateStatic = ScaleAudioBuilder.SAMPLE_RATE
