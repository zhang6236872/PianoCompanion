...
/**
 * 音色辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class TimbreTrainingProgress(
    val stats: MutableMap<String, TimbreTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { TimbreTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: TimbreTrainingDifficulty): TimbreTrainingProgressEntry =
        stats[difficulty.name] ?: TimbreTrainingProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): TimbreTrainingProgress {
            val result = TimbreTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = TimbreTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class TimbreTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): TimbreTrainingProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = TimbreTrainingProgressEntry()
