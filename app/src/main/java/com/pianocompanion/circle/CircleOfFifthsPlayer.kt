 ...
 * 五度圈音频播放器（Android 层）。
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [CircleOfFifthsAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止。资源使用完毕必须调用 [release]。
 *
 * 复用与 [com.pianocompanion.scale.ScalePlayer] 相同的完成检测机制。
 */
class CircleOfFifthsPlayer(
    private val sampleRate: Int = CircleOfFifthsAudioBuilder.SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "CircleOfFifthsPlayer"
        private const val POLL_INTERVAL_MS = 50L
        private val COMPLETION_TOLERANCE_SAMPLES = sampleRateStatic / 50
        private const val sampleRateStatic = CircleOfFifthsAudioBuilder.SAMPLE_RATE
    }
}
