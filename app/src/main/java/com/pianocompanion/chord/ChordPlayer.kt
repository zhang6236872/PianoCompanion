 ...
 * 和弦音频播放器（Android 层）。
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [ChordAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止、重播。资源使用完毕必须调用 [release]。
 *
 * @param sampleRate 采样率
 */
class ChordPlayer(
    private val sampleRate: Int = ChordAudioBuilder.SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "ChordPlayer"
        private const val POLL_INTERVAL_MS = 50L
        private val COMPLETION_TOLERANCE_SAMPLES = sampleRateStatic / 50
        private const val sampleRateStatic = ChordAudioBuilder.SAMPLE_RATE
    }
}
