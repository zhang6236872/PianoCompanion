...
/**
 * 节奏型听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class RhythmPatternProgress(
    val stats: MutableMap<String, RhythmPatternProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { RhythmPatternProgressEntry() }
    ...
    /** 获取指定组合的进度统计。 */
    fun getProgress(difficulty: RhythmDifficulty, tempo: RhythmTempo): RhythmPatternProgressEntry =
        stats[key(difficulty, tempo)] ?: RhythmPatternProgressEntry()
    ...
    companion object {
        fun key(difficulty: RhythmDifficulty, tempo: RhythmTempo): String =
            "${difficulty.name}_${tempo.name}"

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): RhythmPatternProgress {
            val result = RhythmPatternProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = RhythmPatternProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度+速度组合的进度统计。
 */
data class RhythmPatternProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): RhythmPatternProgressEntry? {
            return try {
