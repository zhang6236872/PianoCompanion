...
/**
 * 节奏型听辨训练音频播放器（Android 层）。
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [RhythmPatternAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止、重播。资源管理：使用完毕必须调用 [release]。
 *
 * @param sampleRate 采样率
 */
class RhythmPatternPlayer(
    private val sampleRate: Int = RhythmPatternAudioBuilder.DEFAULT_SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "RhythmPatternPlayer"
        private const val POLL_INTERVAL_MS = 50L
        private val COMPLETION_TOLERANCE_SAMPLES = sampleRateStatic / 50
        private const val sampleRateStatic = RhythmPatternAudioBuilder.DEFAULT_SAMPLE_RATE
    }
}
