...
/**
 * 力度变化方向辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class DynamicsDirectionProgress(
    val stats: MutableMap<String, DynamicsDirectionProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { DynamicsDirectionProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: DynamicsDirectionDifficulty): DynamicsDirectionProgressEntry =
        stats[difficulty.name] ?: DynamicsDirectionProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): DynamicsDirectionProgress {
            val result = DynamicsDirectionProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = DynamicsDirectionProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class DynamicsDirectionProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): DynamicsDirectionProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = DynamicsDirectionProgressEntry()
