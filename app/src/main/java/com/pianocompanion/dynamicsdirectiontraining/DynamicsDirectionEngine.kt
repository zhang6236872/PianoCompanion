...
/**
 * 力度变化方向辨识训练出题引擎（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 根据 [DynamicsDirectionDifficulty] 生成 [DynamicsDirectionQuestion]：
 *
 * 1. **随机选择主音（tonic）**：旋律围绕主音变化，每次换音高增加变化，但旋律轮廓固定，
 *    使「力度变化方向」成为唯一显著特征（音高不是答题依据）。
 * 2. **从该难度候选集中选正确力度方向**。
...
class DynamicsDirectionEngine(
     ...
     * @return 生成的题目
     */
    fun generate(difficulty: DynamicsDirectionDifficulty): DynamicsDirectionQuestion {
        ...
        return DynamicsDirectionQuestion(
    ...
    companion object {
        ...
        fun withSeed(seed: Long): DynamicsDirectionEngine = DynamicsDirectionEngine(Random(seed))
    }
}
