...
/**
 * 调式音阶色彩对比训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class ModeScaleProgress(
    val stats: MutableMap<String, ModeScaleProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { ModeScaleProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: ModeScaleDifficulty): ModeScaleProgressEntry =
        stats[difficulty.name] ?: ModeScaleProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): ModeScaleProgress {
            val result = ModeScaleProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = ModeScaleProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class ModeScaleProgressEntry(
    ...
    companion object {
         ...
         */
        fun fromJson(json: String): ModeScaleProgressEntry? {
            return try {
