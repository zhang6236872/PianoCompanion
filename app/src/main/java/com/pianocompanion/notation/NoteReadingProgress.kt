...
/**
 * 识谱训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class NoteReadingProgress(
    val stats: MutableMap<String, NoteReadingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { NoteReadingProgressEntry() }
     ...
     * 获取指定组合的进度统计。
     */
    fun getProgress(clef: NoteReadingClef, difficulty: NoteReadingDifficulty): NoteReadingProgressEntry {
        return stats[key(clef, difficulty)] ?: NoteReadingProgressEntry()
    }
    ...
    companion object {
        fun key(clef: NoteReadingClef, difficulty: NoteReadingDifficulty): String =
            "${clef.name}_${difficulty.name}"

        /**
         * 从 JSON 字符串反序列化。
         */
        fun fromJson(json: String): NoteReadingProgress {
            val result = NoteReadingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = NoteReadingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个谱号+难度组合的进度统计。
 */
data class NoteReadingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): NoteReadingProgressEntry? {
            return try {
