 ...
 * 识谱训练音频播放器（Android 层）。
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [NoteReadingAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止、重播。与 EarTrainingPlayer 类似但更简化。
 *
 * 资源管理：使用完毕必须调用 [release]。
 *
 * @param sampleRate 采样率
 */
class NoteReadingPlayer(
    private val sampleRate: Int = NoteReadingAudioBuilder.SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "NoteReadingPlayer"
        private const val POLL_INTERVAL_MS = 50L
        private val COMPLETION_TOLERANCE_SAMPLES = sampleRateStatic / 50
        private const val sampleRateStatic = NoteReadingAudioBuilder.SAMPLE_RATE
    }
}
