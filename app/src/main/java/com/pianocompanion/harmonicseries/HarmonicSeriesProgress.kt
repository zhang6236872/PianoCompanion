...
/**
 * 泛音列辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class HarmonicSeriesProgress(
    val stats: MutableMap<String, HarmonicSeriesProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { HarmonicSeriesProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: HarmonicDifficulty): HarmonicSeriesProgressEntry =
        stats[difficulty.name] ?: HarmonicSeriesProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): HarmonicSeriesProgress {
            val result = HarmonicSeriesProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = HarmonicSeriesProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class HarmonicSeriesProgressEntry(
    ...
    companion object {
         ...
         */
        fun fromJson(json: String): HarmonicSeriesProgressEntry? {
            return try {
                ...
                val entry = HarmonicSeriesProgressEntry()
