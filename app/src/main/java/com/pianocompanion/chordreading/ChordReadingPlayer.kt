...
/**
 * 和弦识别训练音频播放器（Android 层）。
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [ChordReadingAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止、重播。
 *
 ...
 */
class ChordReadingPlayer(
    private val sampleRate: Int = ChordReadingAudioBuilder.SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "ChordReadingPlayer"
        private const val POLL_INTERVAL_MS = 50L
        private const val sampleRateStatic = ChordReadingAudioBuilder.SAMPLE_RATE
