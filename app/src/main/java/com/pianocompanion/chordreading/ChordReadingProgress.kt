...
/**
 * 和弦识别训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class ChordReadingProgress(
    val stats: MutableMap<String, ChordReadingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { ChordReadingProgressEntry() }
     ...
     * 获取指定组合的进度统计。
     */
    fun getProgress(clef: ChordReadingClef, difficulty: ChordReadingDifficulty): ChordReadingProgressEntry {
        return stats[key(clef, difficulty)] ?: ChordReadingProgressEntry()
    }
    ...
    companion object {
        fun key(clef: ChordReadingClef, difficulty: ChordReadingDifficulty): String =
            "${clef.name}_${difficulty.name}"

        /**
         * 从 JSON 字符串反序列化。
         */
        fun fromJson(json: String): ChordReadingProgress {
            val result = ChordReadingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = ChordReadingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个谱号+难度组合的进度统计。
 */
data class ChordReadingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): ChordReadingProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = ChordReadingProgressEntry()
