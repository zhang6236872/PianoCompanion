...
/**
 * 强拍 / 重音辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class AccentProgress(
    val stats: MutableMap<String, AccentProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { AccentProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: AccentDifficulty): AccentProgressEntry =
        stats[difficulty.name] ?: AccentProgressEntry()
 ...
 * 单个难度的进度统计。
 */
data class AccentProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): AccentProgressEntry? {
            return try {
