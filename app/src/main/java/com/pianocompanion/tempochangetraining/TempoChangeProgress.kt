...
/**
 * 速度变化方向辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class TempoChangeProgress(
    val stats: MutableMap<String, TempoChangeProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { TempoChangeProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: TempoChangeDifficulty): TempoChangeProgressEntry =
        stats[difficulty.name] ?: TempoChangeProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): TempoChangeProgress {
            val result = TempoChangeProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = TempoChangeProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class TempoChangeProgressEntry(
    ...
    companion object {
         ...
         */
        fun fromJson(json: String): TempoChangeProgressEntry? {
            return try {
