...
/**
 * 节奏细分听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class SubdivisionProgress(
    val stats: MutableMap<String, SubdivisionProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { SubdivisionProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: SubdivisionDifficulty): SubdivisionProgressEntry =
        stats[difficulty.name] ?: SubdivisionProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): SubdivisionProgress {
            val result = SubdivisionProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = SubdivisionProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class SubdivisionProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): SubdivisionProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                // 必须包含全部 5 个字段；缺失任一字段视为不完整数据，返回 null（不计入）。
                val totalAnswered = extractLong(json, "totalAnswered") ?: return null
                val totalCorrect = extractLong(json, "totalCorrect") ?: return null
                val sessionCount = extractLong(json, "sessionCount") ?: return null
                val bestStreak = extractLong(json, "bestStreak") ?: return null
                val bestAccuracy = extractDouble(json, "bestAccuracy") ?: return null
                SubdivisionProgressEntry(
        ...
        /** 键存在返回其值（缺失/解析失败时返回 null），键存在但值非法（负数）时回退 0。 */
        private fun extractLong(json: String, key: String): Long? {
            ...
            // 负数视为非法值，回退 0
            return (sb.toString().toLongOrNull() ?: 0L).coerceAtLeast(0L)
        }

        /** 键存在返回其值（缺失时返回 null），键存在但值非法（负数）时回退 0.0。 */
        private fun extractDouble(json: String, key: String): Double? {
            ...
            // 负数视为非法值，回退 0.0
            return (sb.toString().toDoubleOrNull() ?: 0.0).coerceAtLeast(0.0)
        }
    }
}
