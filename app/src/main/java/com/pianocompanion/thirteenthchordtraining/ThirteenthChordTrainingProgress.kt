...
/**
 * 十三和弦色彩听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class ThirteenthChordTrainingProgress(
    val stats: MutableMap<String, ThirteenthChordTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { ThirteenthChordTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: ThirteenthChordDifficulty): ThirteenthChordTrainingProgressEntry =
        stats[key(difficulty)] ?: ThirteenthChordTrainingProgressEntry()
    ...
    companion object {
        fun key(difficulty: ThirteenthChordDifficulty): String = difficulty.name

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): ThirteenthChordTrainingProgress {
            val result = ThirteenthChordTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = ThirteenthChordTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class ThirteenthChordTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): ThirteenthChordTrainingProgressEntry? {
            return try {
