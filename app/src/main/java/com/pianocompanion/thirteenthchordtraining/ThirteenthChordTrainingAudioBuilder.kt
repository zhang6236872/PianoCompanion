...
/**
 * 十三和弦色彩听辨训练音频构建器（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 将 [ThirteenthChordQuestion] 的十三和弦渲染为可直接播放的 PCM Float 缓冲区（[-1.0, 1.0]）。
 * 复用 [PianoToneSynthesizer] 合成钢琴音色。
 *
 * 渲染流程：
 * 1. 和弦的 7 个音符用 PianoToneSynthesizer 分别合成（同时播放 = 柱式和弦/block chord）
 * 2. 多个音符按时间轴叠加混合（同一时刻开始）
 * 3. 软限幅防止叠加时削波（7 个音叠加极可能超过 1.0）
 *
 * @param synth 音色合成器实例
 * @param sampleRate 采样率
 */
class ThirteenthChordTrainingAudioBuilder(
     ...
     * 为题目渲染音频（柱式十三和弦）。
     */
    fun render(question: ThirteenthChordQuestion): FloatArray {
        return renderChord(question.midiNotes)
    }

    /**
     * 将十三和弦渲染为 PCM 采样（柱式和弦 = 所有音同时发声）。
     *
     * @param midiNotes 和弦的 MIDI 音符号列表（7 个音，从低到高）
     ...
     */
    fun estimateDurationMs(question: ThirteenthChordQuestion): Long {
    ...
    companion object {
        ...
        /** 和弦持续时间（毫秒）。十三和弦有 7 个音，色彩极度丰富，需要足够长让用户辨识。 */
        const val CHORD_DURATION_MS = 2500L
