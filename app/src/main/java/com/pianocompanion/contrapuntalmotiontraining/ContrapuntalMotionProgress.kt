...
/**
 * 声部运动辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class ContrapuntalMotionProgress(
    val stats: MutableMap<String, ContrapuntalMotionProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
     ...
     */
    fun recordSession(
        difficulty: ContrapuntalMotionDifficulty,
        correct: Int,
        total: Int,
        bestStreak: Int
    ) {
        val key = difficulty.name
        val entry = stats.getOrPut(key) { ContrapuntalMotionProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: ContrapuntalMotionDifficulty): ContrapuntalMotionProgressEntry =
        stats[difficulty.name] ?: ContrapuntalMotionProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): ContrapuntalMotionProgress {
            val result = ContrapuntalMotionProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = ContrapuntalMotionProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class ContrapuntalMotionProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): ContrapuntalMotionProgressEntry? {
            return try {
