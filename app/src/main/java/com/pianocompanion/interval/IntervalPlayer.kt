...
/**
 * 音程识别训练音频播放器（Android 层）。
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [IntervalAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止、重播。
 *
...
class IntervalPlayer(
    private val sampleRate: Int = IntervalAudioBuilder.SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "IntervalPlayer"
        private const val POLL_INTERVAL_MS = 50L
        private const val sampleRateStatic = IntervalAudioBuilder.SAMPLE_RATE
