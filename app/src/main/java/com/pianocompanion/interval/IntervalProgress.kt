...
/**
 * 音程识别训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
...
data class IntervalProgress(
    val stats: MutableMap<String, IntervalProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { IntervalProgressEntry() }
     ...
     * 获取指定组合的进度统计。
     */
    fun getProgress(clef: IntervalClef, difficulty: IntervalDifficulty): IntervalProgressEntry {
        return stats[key(clef, difficulty)] ?: IntervalProgressEntry()
    }
    ...
    companion object {
        fun key(clef: IntervalClef, difficulty: IntervalDifficulty): String =
            "${clef.name}_${difficulty.name}"

        /**
         * 从 JSON 字符串反序列化。
         */
        fun fromJson(json: String): IntervalProgress {
            val result = IntervalProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = IntervalProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个谱号+难度组合的进度统计。
 */
data class IntervalProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): IntervalProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = IntervalProgressEntry()
