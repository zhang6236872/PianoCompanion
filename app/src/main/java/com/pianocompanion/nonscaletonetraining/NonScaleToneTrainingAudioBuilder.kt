...
/**
 * 调外音听辨训练音频构建器（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 将 [NonScaleToneQuestion] 的旋律短句（5 个上行音）渲染为可直接播放的 PCM Float 缓冲区
 * （[-1.0, 1.0]）。复用 [PianoToneSynthesizer] 合成钢琴音色，**依次**弹奏旋律各音（melodic，
 * 非柱式和弦）。
 *
 * 渲染流程：
 * 1. 每个音符用 PianoToneSynthesizer 合成（加法合成 + 指数衰减包络）
 * 2. 按时间轴将各音符采样放置到各自的起始位置（依次排列，前一个结束后下一个立即开始）
 * 3. 软限幅防止叠加时削波
 *
 * 每个音符持续时间较长（[NOTE_DURATION_MS]），以便用户清楚听到每个音、辨别调外音。
 *
 * @param synth 音色合成器实例
 * @param sampleRate 采样率
 */
class NonScaleToneTrainingAudioBuilder(
     ...
     * 为题目渲染音频（旋律短句）。
     */
    fun render(question: NonScaleToneQuestion): FloatArray {
        return renderNotes(question.midiNotes)
    }
    ...
    fun renderNotes(
        ...
        val noteDurationSamples = (sampleRate * NOTE_DURATION_MS / 1000.0).toInt()
    ...
    fun estimateDurationMs(question: NonScaleToneQuestion): Long {
    ...
    companion object {
        ...
        /** 每个音符的持续时间（毫秒）。调外音听辨需要用户逐音辨识，采用较慢的速度。 */
        const val NOTE_DURATION_MS = 550L

        /** 前导静音（毫秒）。 */
        const val LEAD_SILENCE_MS = 250L

        /** 尾部静音（毫秒）。 */
        const val TAIL_SILENCE_MS = 450L
