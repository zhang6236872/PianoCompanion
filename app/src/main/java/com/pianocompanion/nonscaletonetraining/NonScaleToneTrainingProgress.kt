...
/**
 * 调外音听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 记录跨会话的练习进度：每种难度的累计统计。
 * 通过 [toJson] / [fromJson] 进行 JSON 序列化（手动实现，无外部依赖，容错解析）。
 *
 * @param stats 各难度的统计映射，键格式为难度名（如 "BEGINNER"）
 */
data class NonScaleToneTrainingProgress(
    val stats: MutableMap<String, NonScaleToneTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val key = key(difficulty)
        val entry = stats.getOrPut(key) { NonScaleToneTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: NonScaleToneDifficulty): NonScaleToneTrainingProgressEntry =
        stats[key(difficulty)] ?: NonScaleToneTrainingProgressEntry()
    ...
    companion object {
        fun key(difficulty: NonScaleToneDifficulty): String = difficulty.name

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): NonScaleToneTrainingProgress {
            val result = NonScaleToneTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = NonScaleToneTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class NonScaleToneTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): NonScaleToneTrainingProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = NonScaleToneTrainingProgressEntry()
