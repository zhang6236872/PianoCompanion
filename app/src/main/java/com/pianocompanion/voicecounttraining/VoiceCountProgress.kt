...
/**
 * 声部数量听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class VoiceCountProgress(
    val stats: MutableMap<String, VoiceCountProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { VoiceCountProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: VoiceCountDifficulty): VoiceCountProgressEntry =
        stats[difficulty.name] ?: VoiceCountProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): VoiceCountProgress {
            val result = VoiceCountProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = VoiceCountProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class VoiceCountProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): VoiceCountProgressEntry? {
            return try {
        ...
        /** 键存在返回其值（缺失/解析失败时返回 null），键存在但值非法时回退 0。 */
        ...
        /** 键存在返回其值（缺失时返回 null），键存在但值非法时回退 0.0。 */
