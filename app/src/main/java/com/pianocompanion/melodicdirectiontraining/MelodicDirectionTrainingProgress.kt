...
/**
 * 旋律方向辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class MelodicDirectionProgress(
    val stats: MutableMap<String, MelodicDirectionProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { MelodicDirectionProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: MelodicDirectionDifficulty): MelodicDirectionProgressEntry =
        stats[difficulty.name] ?: MelodicDirectionProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): MelodicDirectionProgress {
            val result = MelodicDirectionProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = MelodicDirectionProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class MelodicDirectionProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): MelodicDirectionProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = MelodicDirectionProgressEntry()
