...
/**
 * 拍号听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 记录跨会话的练习进度：每种难度 + 速度的累计统计。
 * 通过 [toJson] / [fromJson] 进行 JSON 序列化（手动实现，无外部依赖，容错解析）。
 *
 * @param stats 各难度+速度的统计映射，键格式为 "BEGINNER_SLOW"
 */
data class MeterRecognitionProgress(
    val stats: MutableMap<String, MeterRecognitionProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val key = key(difficulty, tempo)
        val entry = stats.getOrPut(key) { MeterRecognitionProgressEntry() }
    ...
    /** 获取指定组合的进度统计。 */
    fun getProgress(difficulty: MeterRecognitionDifficulty, tempo: MeterRecognitionTempo): MeterRecognitionProgressEntry =
        stats[key(difficulty, tempo)] ?: MeterRecognitionProgressEntry()
    ...
    companion object {
        fun key(difficulty: MeterRecognitionDifficulty, tempo: MeterRecognitionTempo): String =
            "${difficulty.name}_${tempo.name}"

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): MeterRecognitionProgress {
            val result = MeterRecognitionProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = MeterRecognitionProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度+速度组合的进度统计。
 */
data class MeterRecognitionProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): MeterRecognitionProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = MeterRecognitionProgressEntry()
