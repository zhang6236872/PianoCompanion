...
/**
 * 旋律轮廓辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class ContourProgress(
    val stats: MutableMap<String, ContourProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { ContourProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: ContourDifficulty): ContourProgressEntry =
        stats[difficulty.name] ?: ContourProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): ContourProgress {
            val result = ContourProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = ContourProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class ContourProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): ContourProgressEntry? {
            return try {
