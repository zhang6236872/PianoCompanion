...
/**
 * 旋律轮廓辨识训练音频播放器（Android 层）。
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [ContourAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止、重播。资源管理：使用完毕必须调用 [release]。
 *
 * @param sampleRate 采样率
 */
class ContourPlayer(
    private val sampleRate: Int = ContourAudioBuilder.DEFAULT_SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "ContourPlayer"
        private const val POLL_INTERVAL_MS = 50L
        private const val sampleRateStatic = ContourAudioBuilder.DEFAULT_SAMPLE_RATE
