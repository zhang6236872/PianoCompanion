...
/**
 * 转调辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class ModulationProgress(
    val stats: MutableMap<String, ModulationProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
     ...
     */
    fun recordSession(
        difficulty: ModulationDifficulty,
        correct: Int,
        total: Int,
        bestStreak: Int
    ) {
        val key = difficulty.name
        val entry = stats.getOrPut(key) { ModulationProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: ModulationDifficulty): ModulationProgressEntry =
        stats[difficulty.name] ?: ModulationProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): ModulationProgress {
            val result = ModulationProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = ModulationProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class ModulationProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): ModulationProgressEntry? {
            return try {
