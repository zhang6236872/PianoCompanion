...
/**
 * 九和弦色彩听辨训练出题引擎（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 根据 [NinthChordDifficulty] 生成 [NinthChordQuestion]：
 * 1. 从该难度的可用九和弦品质集合中随机选择一个品质
 ...
 * @param root 底层随机数生成器
 */
class NinthChordTrainingEngine(
     ...
     * @return 生成的题目
     */
    fun generate(difficulty: NinthChordDifficulty): NinthChordQuestion {
        val availableQualities = NinthChordQuality.forDifficulty(difficulty)

        val quality = availableQualities.random(root)

        // 随机选择根音，限定在 C3-G3 范围（保证 5 音九和弦在钢琴音域内）
        val rootPc = ROOT_PITCH_CLASSES.random(root)
        val rootMidi = BASE_OCTAVE_MIDI + rootPc
        val rootName = ROOT_NAMES[rootPc] ?: "C"

        // 构建九和弦 MIDI 音符列表（从低到高排列）
        val midiNotes = buildNinthChordMidiNotes(quality, rootMidi)
    ...
    companion object {
        ...
        /** 可用根音音级类（C, D, E, F, G），限定最高到 G 保证九音在音域内。 */
        ...
        fun withSeed(seed: Long): NinthChordTrainingEngine =
            NinthChordTrainingEngine(Random(seed))

        /**
         * 根据九和弦品质构建 MIDI 音符列表。
         *
         * 九和弦音程 [0, i1, i2, i3, i4]（根音、三音、五音、七音、九音偏移）。
         * 根音在最低处，所有音符按半音偏移叠加。
         *
         ...
         */
        fun buildNinthChordMidiNotes(
            quality: NinthChordQuality,
            rootMidi: Int
        ): List<Int> {
