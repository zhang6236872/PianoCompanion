...
/**
 * 九和弦色彩听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class NinthChordTrainingProgress(
    val stats: MutableMap<String, NinthChordTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { NinthChordTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: NinthChordDifficulty): NinthChordTrainingProgressEntry =
        stats[key(difficulty)] ?: NinthChordTrainingProgressEntry()
    ...
    companion object {
        fun key(difficulty: NinthChordDifficulty): String = difficulty.name

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): NinthChordTrainingProgress {
            val result = NinthChordTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = NinthChordTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class NinthChordTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): NinthChordTrainingProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = NinthChordTrainingProgressEntry()
