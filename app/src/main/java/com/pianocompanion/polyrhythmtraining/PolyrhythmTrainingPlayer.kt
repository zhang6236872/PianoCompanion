...
/**
 * 复合节奏辨识训练音频播放器（Android 层）。
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [PolyrhythmTrainingAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止、重播。资源管理：使用完毕必须调用 [release]。
 *
 * @param sampleRate 采样率
 */
class PolyrhythmTrainingPlayer(
    private val sampleRate: Int = PolyrhythmTrainingAudioBuilder.DEFAULT_SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "PolyrhythmPlayer"
        private const val POLL_INTERVAL_MS = 50L
        private const val sampleRateStatic = PolyrhythmTrainingAudioBuilder.DEFAULT_SAMPLE_RATE
