...
/**
 * 复合节奏辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class PolyrhythmTrainingProgress(
    val stats: MutableMap<String, PolyrhythmProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { PolyrhythmProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: PolyrhythmDifficulty): PolyrhythmProgressEntry =
        stats[difficulty.name] ?: PolyrhythmProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): PolyrhythmTrainingProgress {
            val result = PolyrhythmTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = PolyrhythmProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class PolyrhythmProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): PolyrhythmProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = PolyrhythmProgressEntry()
