...
/**
 * 调号识别训练音频播放器（Android 层）。
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [KeySigAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止、重播。
 *
 ...
 */
class KeySigPlayer(
    private val sampleRate: Int = KeySigAudioBuilder.SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "KeySigPlayer"
        private const val POLL_INTERVAL_MS = 50L
        private const val sampleRateStatic = KeySigAudioBuilder.SAMPLE_RATE
