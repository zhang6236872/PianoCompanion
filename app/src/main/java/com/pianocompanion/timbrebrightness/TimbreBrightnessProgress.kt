...
/**
 * 音色亮度辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class TimbreBrightnessProgress(
    val stats: MutableMap<String, TimbreBrightnessProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
     ...
     */
    fun recordSession(
        difficulty: TimbreBrightnessDifficulty,
        correct: Int,
        total: Int,
        bestStreak: Int
    ) {
        val key = difficulty.name
        val entry = stats.getOrPut(key) { TimbreBrightnessProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: TimbreBrightnessDifficulty): TimbreBrightnessProgressEntry =
        stats[difficulty.name] ?: TimbreBrightnessProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): TimbreBrightnessProgress {
            val result = TimbreBrightnessProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = TimbreBrightnessProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class TimbreBrightnessProgressEntry(
    ...
    companion object {
         ...
         */
        fun fromJson(json: String): TimbreBrightnessProgressEntry? {
            return try {
                ...
                val entry = TimbreBrightnessProgressEntry()
