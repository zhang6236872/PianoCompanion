...
/**
 * 速度辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class TempoTrainingProgress(
    val stats: MutableMap<String, TempoTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { TempoTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: TempoTrainingDifficulty): TempoTrainingProgressEntry =
        stats[difficulty.name] ?: TempoTrainingProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): TempoTrainingProgress {
            val result = TempoTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = TempoTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class TempoTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): TempoTrainingProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = TempoTrainingProgressEntry()
