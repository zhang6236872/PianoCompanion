...
/**
 * 和弦功能听辨训练音频构建器（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 将 [ChordFunctionQuestion] 的和弦渲染为可直接播放的 PCM Float 缓冲区（[-1.0, 1.0]）。
 ...
 */
class ChordFunctionTrainingAudioBuilder(
     ...
     * 为题目渲染音频（柱式和弦）。
     */
    fun render(question: ChordFunctionQuestion): FloatArray {
    ...
    fun estimateDurationMs(question: ChordFunctionQuestion): Long {
