...
/**
 * 和弦功能听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class ChordFunctionTrainingProgress(
    val stats: MutableMap<String, ChordFunctionTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { ChordFunctionTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: ChordFunctionDifficulty): ChordFunctionTrainingProgressEntry =
        stats[key(difficulty)] ?: ChordFunctionTrainingProgressEntry()
    ...
    companion object {
        fun key(difficulty: ChordFunctionDifficulty): String = difficulty.name

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): ChordFunctionTrainingProgress {
            val result = ChordFunctionTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = ChordFunctionTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class ChordFunctionTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): ChordFunctionTrainingProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = ChordFunctionTrainingProgressEntry()
