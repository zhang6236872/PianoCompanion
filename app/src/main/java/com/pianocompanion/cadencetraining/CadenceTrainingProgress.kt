...
/**
 * 终止式听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class CadenceTrainingProgress(
    val stats: MutableMap<String, CadenceTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { CadenceTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: CadenceDifficulty): CadenceTrainingProgressEntry =
        stats[key(difficulty)] ?: CadenceTrainingProgressEntry()
    ...
    companion object {
        fun key(difficulty: CadenceDifficulty): String = difficulty.name

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): CadenceTrainingProgress {
            val result = CadenceTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = CadenceTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class CadenceTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): CadenceTrainingProgressEntry? {
            return try {
