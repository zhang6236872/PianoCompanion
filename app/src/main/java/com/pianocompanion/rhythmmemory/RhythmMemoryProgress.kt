...
/**
 * 节奏型记忆训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 记录跨会话的练习进度：每种难度的累计统计。
 * 通过 [toJson] / [fromJson] 进行 JSON 序列化（手动实现，无外部依赖，容错解析）。
 *
 * @param stats 各难度的统计映射，键格式为难度枚举名（如 "BEGINNER"）
 */
data class RhythmMemoryProgress(
    val stats: MutableMap<String, RhythmMemoryProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { RhythmMemoryProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: RhythmMemoryDifficulty): RhythmMemoryProgressEntry =
        stats[difficulty.name] ?: RhythmMemoryProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): RhythmMemoryProgress {
            val result = RhythmMemoryProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = RhythmMemoryProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class RhythmMemoryProgressEntry(
    ...
    companion object {
         ...
         * 从 JSON 反序列化。
         *
         * **严格解析**：必须包含全部 5 个字段，缺失任一字段视为不完整数据返回 null；
         * 负数回退 0（coerceAtLeast）避免错误统计。
         */
        fun fromJson(json: String): RhythmMemoryProgressEntry? {
            return try {
                ...
                val entry = RhythmMemoryProgressEntry()
