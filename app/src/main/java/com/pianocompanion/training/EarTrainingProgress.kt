...
/**
 * 听音训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 记录跨会话的练习进度：每种练习类型 + 难度组合的累计统计。
 * 通过 [toJson] / [fromJson] 进行 JSON 序列化（Gson 兼容格式）。
 *
 * @param stats 各练习类型+难度的统计映射，键格式为 "INTERVAL_BEGINNER"
 */
data class EarTrainingProgress(
    val stats: MutableMap<String, ProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val key = key(exerciseType, difficulty)
        val entry = stats.getOrPut(key) { ProgressEntry() }
     ...
     * 获取指定组合的进度统计。
     */
    fun getProgress(exerciseType: ExerciseType, difficulty: Difficulty): ProgressEntry {
        return stats[key(exerciseType, difficulty)] ?: ProgressEntry()
    }
    ...
    /**
     * 序列化为 JSON 字符串（Gson 兼容的手动实现，无外部依赖）。
     */
    fun toJson(): String {
    ...
    companion object {
        fun key(exerciseType: ExerciseType, difficulty: Difficulty): String =
            "${exerciseType.name}_${difficulty.name}"

        /**
         * 从 JSON 字符串反序列化。
         */
        fun fromJson(json: String): EarTrainingProgress {
            val result = EarTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                // 解析键值对
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = ProgressEntry.fromJson(valueStr)
                    if (entry != null) {
...
data class ProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): ProgressEntry? {
            return try {
