...
/**
 * 和弦进行听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class ProgressionTrainingProgress(
    val stats: MutableMap<String, ProgressionTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { ProgressionTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: ProgressionDifficulty): ProgressionTrainingProgressEntry =
        stats[key(difficulty)] ?: ProgressionTrainingProgressEntry()
    ...
    companion object {
        fun key(difficulty: ProgressionDifficulty): String = difficulty.name

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): ProgressionTrainingProgress {
            val result = ProgressionTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = ProgressionTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class ProgressionTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): ProgressionTrainingProgressEntry? {
            return try {
