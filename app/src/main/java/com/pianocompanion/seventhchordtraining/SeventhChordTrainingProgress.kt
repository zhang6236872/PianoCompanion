...
/**
 * 七和弦品质听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class SeventhChordTrainingProgress(
    val stats: MutableMap<String, SeventhChordTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { SeventhChordTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: SeventhChordDifficulty): SeventhChordTrainingProgressEntry =
        stats[key(difficulty)] ?: SeventhChordTrainingProgressEntry()
    ...
    companion object {
        fun key(difficulty: SeventhChordDifficulty): String = difficulty.name

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): SeventhChordTrainingProgress {
            val result = SeventhChordTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = SeventhChordTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class SeventhChordTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): SeventhChordTrainingProgressEntry? {
            return try {
