...
/**
 * 和声色彩听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class HarmonyColorProgress(
    val stats: MutableMap<String, HarmonyColorProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
     ...
     */
    fun recordSession(
        difficulty: HarmonyColorDifficulty,
        correct: Int,
        total: Int,
        bestStreak: Int
    ) {
        val key = difficulty.name
        val entry = stats.getOrPut(key) { HarmonyColorProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: HarmonyColorDifficulty): HarmonyColorProgressEntry =
        stats[difficulty.name] ?: HarmonyColorProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): HarmonyColorProgress {
            val result = HarmonyColorProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = HarmonyColorProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class HarmonyColorProgressEntry(
    ...
    companion object {
         ...
         */
        fun fromJson(json: String): HarmonyColorProgressEntry? {
            return try {
                ...
                val entry = HarmonyColorProgressEntry()
