...
/**
 * 动机发展辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class MotifTransformationProgress(
    val stats: MutableMap<String, MotifTransformationProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
     ...
     */
    fun recordSession(
        difficulty: MotifTransformationDifficulty,
        correct: Int,
        total: Int,
        bestStreak: Int
    ) {
        val key = difficulty.name
        val entry = stats.getOrPut(key) { MotifTransformationProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: MotifTransformationDifficulty): MotifTransformationProgressEntry =
        stats[difficulty.name] ?: MotifTransformationProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): MotifTransformationProgress {
            val result = MotifTransformationProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = MotifTransformationProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class MotifTransformationProgressEntry(
    ...
    companion object {
         ...
         * 从 JSON 反序列化。
         *
         * **严格解析**：必须包含全部 5 个字段（totalAnswered/totalCorrect/sessionCount/
         * bestStreak/bestAccuracy），缺失任一字段视为不完整数据返回 null 不计入；
         * 负数回退 0（coerceAtLeast）避免错误统计。
         */
        fun fromJson(json: String): MotifTransformationProgressEntry? {
            return try {
                ...
                val required = listOf(
                    ...
                    "bestStreak", "bestAccuracy"
                )
                if (required.any { !json.contains("\"${it}\"") }) return null
                val entry = MotifTransformationProgressEntry()
                entry.totalAnswered = extractLong(json, "totalAnswered").toInt().coerceAtLeast(0)
                entry.totalCorrect = extractLong(json, "totalCorrect").toInt().coerceAtLeast(0)
                entry.sessionCount = extractLong(json, "sessionCount").toInt().coerceAtLeast(0)
                entry.bestStreak = extractLong(json, "bestStreak").toInt().coerceAtLeast(0)
                entry.bestAccuracy = extractDouble(json, "bestAccuracy").coerceAtLeast(0.0)
                entry
            } catch (_: Exception) {
                null
            }
