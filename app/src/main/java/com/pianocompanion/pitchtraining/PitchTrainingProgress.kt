...
/**
 * 绝对音高训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class PitchTrainingProgress(
    val stats: MutableMap<String, PitchTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { PitchTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: PitchTrainingDifficulty): PitchTrainingProgressEntry =
        stats[key(difficulty)] ?: PitchTrainingProgressEntry()
    ...
    companion object {
        fun key(difficulty: PitchTrainingDifficulty): String = difficulty.name

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): PitchTrainingProgress {
            val result = PitchTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = PitchTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class PitchTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): PitchTrainingProgressEntry? {
            return try {
