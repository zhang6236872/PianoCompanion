...
/**
 * 和声音程辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class HarmonicIntervalProgress(
    val stats: MutableMap<String, HarmonicIntervalProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { HarmonicIntervalProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: HarmonicIntervalDifficulty): HarmonicIntervalProgressEntry =
        stats[difficulty.name] ?: HarmonicIntervalProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): HarmonicIntervalProgress {
            val result = HarmonicIntervalProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = HarmonicIntervalProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class HarmonicIntervalProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): HarmonicIntervalProgressEntry? {
            return try {
