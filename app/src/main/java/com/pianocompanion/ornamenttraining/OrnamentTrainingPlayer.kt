...
/**
 * 装饰音辨识训练音频播放器（Android 层）。
 *
 * 使用 [AudioTrack]（MODE_STATIC）播放 [OrnamentTrainingAudioBuilder] 渲染的 PCM 缓冲区。
 * 支持播放、停止、重播。资源管理：使用完毕必须调用 [release]。
 *
 * @param sampleRate 采样率
 */
class OrnamentTrainingPlayer(
    private val sampleRate: Int = OrnamentTrainingAudioBuilder.SAMPLE_RATE
) {
    ...
    companion object {
        private const val TAG = "OrnamentTrainingPlayer"
        private const val POLL_INTERVAL_MS = 50L
        private const val sampleRateStatic = OrnamentTrainingAudioBuilder.SAMPLE_RATE
