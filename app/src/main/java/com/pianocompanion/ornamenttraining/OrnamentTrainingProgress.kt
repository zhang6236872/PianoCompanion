...
/**
 * 装饰音辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class OrnamentTrainingProgress(
    val stats: MutableMap<String, OrnamentTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { OrnamentTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: OrnamentDifficulty): OrnamentTrainingProgressEntry =
        stats[key(difficulty)] ?: OrnamentTrainingProgressEntry()
    ...
    companion object {
        fun key(difficulty: OrnamentDifficulty): String = difficulty.name

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): OrnamentTrainingProgress {
            val result = OrnamentTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = OrnamentTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class OrnamentTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): OrnamentTrainingProgressEntry? {
            return try {
