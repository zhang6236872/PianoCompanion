...
/**
 * 旋律记忆训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class MelodyMemoryProgress(
    val stats: MutableMap<String, MelodyMemoryProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { MelodyMemoryProgressEntry() }
    ...
    /** 获取指定组合的进度统计。 */
    fun getProgress(difficulty: MelodyDifficulty, tempo: MelodyTempo): MelodyMemoryProgressEntry =
        stats[key(difficulty, tempo)] ?: MelodyMemoryProgressEntry()
    ...
    companion object {
        fun key(difficulty: MelodyDifficulty, tempo: MelodyTempo): String =
            "${difficulty.name}_${tempo.name}"

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): MelodyMemoryProgress {
            val result = MelodyMemoryProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = MelodyMemoryProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度+速度组合的进度统计。
 */
data class MelodyMemoryProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): MelodyMemoryProgressEntry? {
            return try {
