...
/**
 * 复合节拍听辨训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class CompoundMeterProgress(
    val stats: MutableMap<String, CompoundMeterProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { CompoundMeterProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: CompoundMeterDifficulty): CompoundMeterProgressEntry =
        stats[difficulty.name] ?: CompoundMeterProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): CompoundMeterProgress {
            val result = CompoundMeterProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = CompoundMeterProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class CompoundMeterProgressEntry(
    ...
    companion object {
         ...
         */
        fun fromJson(json: String): CompoundMeterProgressEntry? {
            return try {
                ...
                val entry = CompoundMeterProgressEntry()
