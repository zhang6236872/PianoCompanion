...
/**
 * 调性中心辨识训练音频构建器（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 将 [KeyQuestion] 的旋律序列渲染为可直接播放的 PCM Float 缓冲区（[-1.0, 1.0]）。
 ...
 * 4. 软限幅防止叠加时削波
 *
 * 旋律从主音出发并回归主音，配合特征音（升降号），
 * 让用户能够通过聆听建立调性感、判断调性中心。
 *
 * @param synth 音色合成器实例
 * @param sampleRate 采样率
 */
class KeyIdentificationTrainingAudioBuilder(
     ...
     * 为题目渲染音频（旋律序列）。
     */
    fun render(question: KeyQuestion): FloatArray {
        return renderMelody(question.midiNotes)
    }
    ...
    fun estimateDurationMs(question: KeyQuestion): Long {
