...
/**
 * 织体辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class TextureProgress(
    val stats: MutableMap<String, TextureProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { TextureProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: TextureDifficulty): TextureProgressEntry =
        stats[difficulty.name] ?: TextureProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): TextureProgress {
            val result = TextureProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = TextureProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class TextureProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): TextureProgressEntry? {
            return try {
                val trimmed = json.trim()
                if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return null
                val entry = TextureProgressEntry()
