...
/**
 * 模进辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class SequenceTrainingProgress(
    val stats: MutableMap<String, SequenceTrainingProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { SequenceTrainingProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: SequenceDifficulty): SequenceTrainingProgressEntry =
        stats[key(difficulty)] ?: SequenceTrainingProgressEntry()
    ...
    companion object {
        fun key(difficulty: SequenceDifficulty): String = difficulty.name

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): SequenceTrainingProgress {
            val result = SequenceTrainingProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = SequenceTrainingProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class SequenceTrainingProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): SequenceTrainingProgressEntry? {
            return try {
