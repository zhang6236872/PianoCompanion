...
/**
 * 织体类型辨识训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class TextureCategoryProgress(
    val stats: MutableMap<String, TextureCategoryProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { TextureCategoryProgressEntry() }
    ...
    /** 获取指定难度的进度统计。 */
    fun getProgress(difficulty: MusicTextureDifficulty): TextureCategoryProgressEntry =
        stats[difficulty.name] ?: TextureCategoryProgressEntry()
    ...
    companion object {
        ...
        fun fromJson(json: String): TextureCategoryProgress {
            val result = TextureCategoryProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = TextureCategoryProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度的进度统计。
 */
data class TextureCategoryProgressEntry(
    ...
    companion object {
         ...
         */
        fun fromJson(json: String): TextureCategoryProgressEntry? {
            return try {
                ...
                val entry = TextureCategoryProgressEntry()
