...
/**
 * 节奏听写训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 ...
 */
data class RhythmDictationProgress(
    val stats: MutableMap<String, RhythmDictationProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val entry = stats.getOrPut(key) { RhythmDictationProgressEntry() }
    ...
    /** 获取指定组合的进度统计。 */
    fun getProgress(difficulty: RhythmDictationDifficulty, tempo: RhythmDictationTempo): RhythmDictationProgressEntry =
        stats[key(difficulty, tempo)] ?: RhythmDictationProgressEntry()
    ...
    companion object {
        fun key(difficulty: RhythmDictationDifficulty, tempo: RhythmDictationTempo): String =
            "${difficulty.name}_${tempo.name}"

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): RhythmDictationProgress {
            val result = RhythmDictationProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = RhythmDictationProgressEntry.fromJson(valueStr)
                    if (entry != null) {
 ...
 * 单个难度+速度组合的进度统计。
 */
data class RhythmDictationProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): RhythmDictationProgressEntry? {
            return try {
