...
/**
 * 十一和弦色彩听辨训练出题引擎（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 根据 [EleventhChordDifficulty] 生成 [EleventhChordQuestion]：
 * 1. 从该难度的可用十一和弦品质集合中随机选择一个品质
 ...
 * @param root 底层随机数生成器
 */
class EleventhChordTrainingEngine(
     ...
     * @return 生成的题目
     */
    fun generate(difficulty: EleventhChordDifficulty): EleventhChordQuestion {
        val availableQualities = EleventhChordQuality.forDifficulty(difficulty)

        val quality = availableQualities.random(root)

        // 随机选择根音，限定在 C3-G3 范围（保证 6 音十一和弦在钢琴音域内）
        val rootPc = ROOT_PITCH_CLASSES.random(root)
        val rootMidi = BASE_OCTAVE_MIDI + rootPc
        val rootName = ROOT_NAMES[rootPc] ?: "C"

        // 构建十一和弦 MIDI 音符列表（从低到高排列）
        val midiNotes = buildEleventhChordMidiNotes(quality, rootMidi)
    ...
    companion object {
        ...
        /** 可用根音音级类（C, D, E, F, G），限定最高到 G 保证十一音在音域内。 */
        ...
        fun withSeed(seed: Long): EleventhChordTrainingEngine =
            EleventhChordTrainingEngine(Random(seed))

        /**
         * 根据十一和弦品质构建 MIDI 音符列表。
         *
         * 十一和弦音程 [0, i1, i2, i3, i4, i5]（根音、三音、五音、七音、九音、十一音偏移）。
         * 根音在最低处，所有音符按半音偏移叠加。
         *
         * 结果始终按音高从低到高排列（MIDI 值递增）。
         *
         * @param quality 十一和弦品质
         * @param rootMidi 根音 MIDI 音符号
         * @return 和弦 MIDI 音符号列表（已钳制到钢琴范围 [21, 108]，从低到高排列）
         */
        fun buildEleventhChordMidiNotes(
            quality: EleventhChordQuality,
            rootMidi: Int
        ): List<Int> {
