...
/**
 * 调式识别训练进度跟踪模型（纯 Kotlin，无 Android 依赖，完全可单元测试）。
 *
 * 记录跨会话的练习进度：每种难度 + 播放方向的累计统计。
 * 通过 [toJson] / [fromJson] 进行 JSON 序列化（手动实现，无外部依赖，容错解析）。
 *
 * @param stats 各难度+播放方向的统计映射，键格式为 "BEGINNER_ASCENDING"
 */
data class ModeRecognitionProgress(
    val stats: MutableMap<String, ModeProgressEntry> = mutableMapOf()
) {
    /**
     * 记录一次会话的结果。
     *
    ...
    fun recordSession(
        ...
        val key = key(difficulty, playMode)
        val entry = stats.getOrPut(key) { ModeProgressEntry() }
    ...
    /** 获取指定组合的进度统计。 */
    fun getProgress(difficulty: ModeDifficulty, playMode: PlayMode): ModeProgressEntry =
        stats[key(difficulty, playMode)] ?: ModeProgressEntry()
    ...
    companion object {
        fun key(difficulty: ModeDifficulty, playMode: PlayMode): String =
            "${difficulty.name}_${playMode.name}"

        /** 从 JSON 字符串反序列化（容错：损坏 JSON 返回空进度）。 */
        fun fromJson(json: String): ModeRecognitionProgress {
            val result = ModeRecognitionProgress()
            try {
                val statsObj = extractObject(json, "stats") ?: return result
                val pairs = splitKeyValuePairs(statsObj)
                for ((key, valueStr) in pairs) {
                    val entry = ModeProgressEntry.fromJson(valueStr)
                    if (entry != null) {
...
data class ModeProgressEntry(
    ...
    companion object {
        fun fromJson(json: String): ModeProgressEntry? {
            return try {
