 ...
 */
class SeventhChordTrainingProgressTest {

    // ── 分难度累计 ────────────────────────────────────────────

    @Test
    fun `recordSession accumulates per difficulty`() {
        val progress = SeventhChordTrainingProgress()
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 3, 5, 2)
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 4, 5, 3)

        val entry = progress.getProgress(SeventhChordDifficulty.BEGINNER)
    ...
    fun `different difficulties are independent`() {
        val progress = SeventhChordTrainingProgress()
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 3, 5, 2)
        progress.recordSession(SeventhChordDifficulty.ADVANCED, 1, 5, 1)

        assertEquals(5, progress.getProgress(SeventhChordDifficulty.BEGINNER).totalAnswered)
        assertEquals(5, progress.getProgress(SeventhChordDifficulty.ADVANCED).totalAnswered)
    }

    @Test
    fun `empty difficulty returns default entry`() {
        val progress = SeventhChordTrainingProgress()
        val entry = progress.getProgress(SeventhChordDifficulty.INTERMEDIATE)
    ...
    fun `totalAnswered sums across difficulties`() {
        val progress = SeventhChordTrainingProgress()
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 3, 5, 2)
        progress.recordSession(SeventhChordDifficulty.ADVANCED, 2, 10, 1)
    ...
    fun `overallAccuracy computes correctly`() {
        val progress = SeventhChordTrainingProgress()
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 5, 10, 3)
    ...
    fun `overallBestStreak takes max across difficulties`() {
        val progress = SeventhChordTrainingProgress()
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 3, 5, 7)
        progress.recordSession(SeventhChordDifficulty.ADVANCED, 2, 5, 4)
    ...
    fun `bestStreak does not decrease`() {
        val progress = SeventhChordTrainingProgress()
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 5, 5, 10)
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 1, 5, 2)
        assertEquals(10, progress.getProgress(SeventhChordDifficulty.BEGINNER).bestStreak)
    }

    @Test
    fun `bestAccuracy does not decrease`() {
        val progress = SeventhChordTrainingProgress()
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 5, 5, 3) // 100%
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 0, 5, 0) // 0%
        assertEquals(1.0, progress.getProgress(SeventhChordDifficulty.BEGINNER).bestAccuracy, 0.001)
    }

    @Test
    fun `bestAccuracy updates when higher`() {
        val progress = SeventhChordTrainingProgress()
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 3, 5, 1) // 60%
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 5, 5, 3) // 100%
        assertEquals(1.0, progress.getProgress(SeventhChordDifficulty.BEGINNER).bestAccuracy, 0.001)
    }
    ...
    fun `json roundtrip preserves data`() {
        val progress = SeventhChordTrainingProgress()
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 3, 5, 2)
        progress.recordSession(SeventhChordDifficulty.INTERMEDIATE, 4, 5, 3)
        progress.recordSession(SeventhChordDifficulty.ADVANCED, 2, 5, 1)

        val json = progress.toJson()
        val restored = SeventhChordTrainingProgress.fromJson(json)
    ...
    fun `json roundtrip preserves per-difficulty data`() {
        val progress = SeventhChordTrainingProgress()
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 7, 10, 5)
        val json = progress.toJson()
        val restored = SeventhChordTrainingProgress.fromJson(json)
        val entry = restored.getProgress(SeventhChordDifficulty.BEGINNER)
        assertEquals(10, entry.totalAnswered)
        assertEquals(7, entry.totalCorrect)
    ...
    fun `fromJson with empty string returns empty progress`() {
        val progress = SeventhChordTrainingProgress.fromJson("")
        assertEquals(0, progress.totalAnswered)
    }

    @Test
    fun `fromJson with garbage returns empty progress`() {
        val progress = SeventhChordTrainingProgress.fromJson("not valid json {{{")
        assertEquals(0, progress.totalAnswered)
    }

    @Test
    fun `fromJson with null-like returns empty progress`() {
        val progress = SeventhChordTrainingProgress.fromJson("null")
        assertEquals(0, progress.totalAnswered)
    }

    @Test
    fun `fromJson with missing stats field returns empty progress`() {
        val progress = SeventhChordTrainingProgress.fromJson("{\"version\":1}")
        assertEquals(0, progress.totalAnswered)
    }
    ...
    fun `fromJson with missing fields in entry defaults to zero`() {
        ...
        val progress = SeventhChordTrainingProgress.fromJson(json)
        val entry = progress.getProgress(SeventhChordDifficulty.BEGINNER)
        assertEquals(5, entry.totalAnswered)
        assertEquals(0, entry.totalCorrect) // missing
        assertEquals(0, entry.sessionCount) // missing
        assertEquals(0.0, entry.bestAccuracy, 0.001) // missing
    }
    ...
    fun `fromJson with partial entries preserves valid ones`() {
        ...
        val progress = SeventhChordTrainingProgress.fromJson(json)
        val beginner = progress.getProgress(SeventhChordDifficulty.BEGINNER)
    ...
    fun `multiple roundtrips are stable`() {
        var progress = SeventhChordTrainingProgress()
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 3, 5, 2)
        progress.recordSession(SeventhChordDifficulty.ADVANCED, 4, 5, 3)

        for (i in 1..5) {
            val json = progress.toJson()
            progress = SeventhChordTrainingProgress.fromJson(json)
        }
    ...
    fun `cumulativeAccuracy computes correctly`() {
        val progress = SeventhChordTrainingProgress()
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 3, 10, 2)
        progress.recordSession(SeventhChordDifficulty.BEGINNER, 5, 10, 4)
        val entry = progress.getProgress(SeventhChordDifficulty.BEGINNER)
    ...
    fun `cumulativeAccuracy is zero when no answers`() {
        val entry = SeventhChordTrainingProgressEntry()
    ...
    fun `key function returns difficulty name`() {
        assertEquals("BEGINNER", SeventhChordTrainingProgress.key(SeventhChordDifficulty.BEGINNER))
        assertEquals("INTERMEDIATE", SeventhChordTrainingProgress.key(SeventhChordDifficulty.INTERMEDIATE))
        assertEquals("ADVANCED", SeventhChordTrainingProgress.key(SeventhChordDifficulty.ADVANCED))
    }
}
